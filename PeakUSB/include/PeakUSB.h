 ...
 *
 * PeakUSB: macOS user-space driver for PEAK PCAN-USB adapters.
 *
 * This header declares the PCAN-Basic API as implemented by libPeakUSB.
 * The API definition, its constants and its structures come from PEAK's
 * own PCANBasic.h, which ships with peak-linux-driver under LGPL-2.1.
 * Section 3 of the LGPL permits using such a copy under the terms of the
 * ordinary GNU General Public License, which is what this library does.
 *
 *   Copyright (C) 1999-2025  PEAK-System Technik GmbH, Darmstadt
 *   PCAN-Basic API, author Keneth Wagner. See http://www.peak-system.com
 *   Copyright (C) 2026  Cliff Rubio (macOS port)
 *
 * Differences from PEAK's header, all forced by the platform or by scope:
 *   - The Windows integer types are mapped onto <stdint.h> rather than
 *     pulled in from <pcan.h>, which is Linux-only.
 *   - PCAN_RECEIVE_EVENT yields a pipe file descriptor usable with poll()
 *     or select(), in place of a Windows event handle.
 *   - Only the PCAN_USBBUS* channels are declared. This library drives USB
 *     adapters; there is no PCI, PC Card or LAN hardware behind it.
 *   - The CAN XL entry points are not declared, as they are not implemented.
 */
#ifndef PEAKUSB_H
#define PEAKUSB_H
...
#define PEAKUSB_LIB_VERSION "1.0.2"

/* Windows integer type names used throughout the PCAN-Basic API. */
#ifndef BYTE
#define BYTE   uint8_t
#endif
#ifndef WORD
...
////////////////////////////////////////////////////////////
// Represents the kind of PCAN device
////////////////////////////////////////////////////////////
...
#define PCAN_DEVICE_GUID              0x30U  // Device unique identifier
...
#define PCAN_BUSSPEED_XL              0x33U  // Configured CAN XL speed in bits per second
...
////////////////////////////////////////////////////////////
// CAN message types and filter modes
////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////
// Parameter names for CAN_InitializeFD bit rate strings
////////////////////////////////////////////////////////////
...
#define TPCANTimestampXL    TPCANTimestampFD // Timestamp of a received PCAN XL message
...
// Represents a PCAN message from a CAN XL capable hardware
// REMARKS: for CAN CC and CAN FD frames only PID, MSGTYPE, DLC and DATA are
// used; the CAN XL specific fields are ignored.
//
typedef struct tagTPCANMsgXL
{
    DWORD               PID;        // CAN XL: priority ID (0..0x7FF)
                                    // CAN CC/FD: 11/29-bit message identifier
    ...
    BYTE                SDT;        // Service Data unit Type
    DWORD               AF;         // Acceptance Field
    BYTE                RRS;        // Remote Request Substitution flag (0..1)
    BYTE                SEC;        // Simple Extended Content flag (0..1)
    BYTE                DATA[2048]; // Data of the message
} TPCANMsgXL;
...
////////////////////////////////////////////////////////////
// PCAN-Basic API function prototypes
////////////////////////////////////////////////////////////
...
// Initializes a PCAN channel for plain CAN communication
//
...
// Initializes a FD capable PCAN channel from a bit rate string
//
TPCANStatus CAN_InitializeFD(
        TPCANHandle Channel,
        TPCANBitrateFD BitrateFD);

// Uninitializes a PCAN channel and releases the adapter
//
...
// Reads one CAN message from the receive queue of a PCAN channel
//
...
// Reads one CAN FD message from the receive queue of a PCAN channel
//
...
// Transmits one CAN message
//
TPCANStatus CAN_Write(
        TPCANHandle Channel,
        TPCANMsg *MessageBuffer);

// Transmits one CAN FD message
//
TPCANStatus CAN_WriteFD(
        TPCANHandle Channel,
        TPCANMsgFD *MessageBuffer);

// Configures the reception filter of a PCAN channel
//
...
// Retrieves a PCAN channel or API value
//
...
// Configures a PCAN channel or API value
//
...
// Initialises a CAN XL capable PCAN channel from a bit rate string.
// Only the PCAN-USB XL supports this; every other adapter returns
// PCAN_ERROR_ILLOPERATION.
//
TPCANStatus CAN_InitializeXL(
        TPCANHandle Channel,
        TPCANBitrateXL BitrateXL);

// Reads one CAN XL message from the receive queue of a PCAN channel
//
TPCANStatus CAN_ReadXL(
        TPCANHandle Channel,
        TPCANMsgXL *MessageBuffer,
        TPCANTimestampXL *TimestampBuffer);

// Transmits one CAN XL message
//
TPCANStatus CAN_WriteXL(
        TPCANHandle Channel,
        TPCANMsgXL *MessageBuffer);

// Looks up a PCAN channel from a parameter string
//
